...
export function EditSchemePanel({
  schemeId,
  initial,
}: {
  schemeId: string
  initial: { name: string; description?: string | null; status: string }
}) {
  ...
  async function handleSave() {
    ...
    setError(null)
    try {
      const res = await fetch(`/api/membership-schemes/${schemeId}`, {
        method: "PATCH",
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm space-y-4">
      <h2 className="text-sm font-semibold text-slate-900">Edit Scheme</h2>
