...
import { getTeamReportCharges, getTeams, getAllCompetitionsForReport, getCompetitionEntriesForReport } from "@/lib/api"
...
export default async function FeeCollectionReportPage({
  ...
  const [charges, teams, competitions] = await Promise.all([
    getTeamReportCharges(seasonFilter).catch(() => []),
    getTeams().catch(() => []),
    getAllCompetitionsForReport().catch(() => [] as any[]),
  ])
  ...
  const allCompEntries: any[] = []
  competitions.forEach((comp: any, i: number) => {
    for (const entry of entriesPerComp[i]) {
      allCompEntries.push({
        ...entry,
  ...
  const compEntriesWithFee = allCompEntries.filter((e) => parseFloat(e.entryFee ?? "0") > 0)
  const compTotalCharged = compEntriesWithFee.reduce((s, e) => s + parseFloat(e.entryFee ?? "0"), 0)
  const compTotalCollected = compEntriesWithFee
    .filter((e) => e.status === "confirmed")
    .reduce((s, e) => s + parseFloat(e.entryFee ?? "0"), 0)
  const compTotalPending = compEntriesWithFee
    .filter((e) => e.status === "pending")
    .reduce((s, e) => s + parseFloat(e.entryFee ?? "0"), 0)
  const compCollectionRate = compTotalCharged > 0
    ? Math.round((compTotalCollected / compTotalCharged) * 100)
    : null

  const compByCompetition: Record<string, { name: string; total: number }> = {}
  for (const e of compEntriesWithFee) {
    const key = e.competitionName
    if (!compByCompetition[key]) compByCompetition[key] = { name: key, total: 0 }
    compByCompetition[key].total += parseFloat(e.entryFee ?? "0")
  }
  const compFeeRows = Object.values(compByCompetition)
    .sort((a, b) => b.total - a.total)
    .map((r) => ({ label: r.name, value: Math.round(r.total), valueFormatted: `£${r.total.toFixed(2)}` }))

  const compStatusSlices = [
    { label: "Confirmed", value: compEntriesWithFee.filter((e) => e.status === "confirmed").length },
    { label: "Pending", value: compEntriesWithFee.filter((e) => e.status === "pending").length },
    { label: "Withdrawn", value: compEntriesWithFee.filter((e) => e.status === "withdrawn" || e.status === "rejected").length },
  ].filter((s) => s.value > 0)
  ...
  return (
    <PortalLayout title="Fee Collection" description="Match fee charges, payment status, and collection rates across all teams.">
      <div className="space-y-6">
        ...
        {compEntriesWithFee.length > 0 && (
          <>
            <div className="mt-2 border-t border-slate-200 pt-6">
              <h2 className="text-lg font-bold text-slate-900">Competition entry fees</h2>
              <p className="mt-1 text-sm text-slate-500">Entry fee collection across all competitions (entries with a fee set).</p>
            </div>

            <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
              <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                <div className="text-sm font-medium text-slate-500">Total entry fees</div>
                <div className="mt-2 text-3xl font-bold text-slate-950">£{compTotalCharged.toFixed(2)}</div>
              </div>
              <div className="rounded-2xl border border-emerald-200 bg-emerald-50/60 p-5 shadow-sm">
                <div className="text-sm font-medium text-emerald-700">Collected</div>
                <div className="mt-2 text-3xl font-bold text-slate-950">£{compTotalCollected.toFixed(2)}</div>
              </div>
              <div className="rounded-2xl border border-amber-200 bg-amber-50/60 p-5 shadow-sm">
                <div className="text-sm font-medium text-amber-700">Outstanding</div>
                <div className="mt-2 text-3xl font-bold text-slate-950">£{compTotalPending.toFixed(2)}</div>
              </div>
              <div className="rounded-2xl border border-sky-200 bg-sky-50/60 p-5 shadow-sm">
                <div className="text-sm font-medium text-sky-700">Collection rate</div>
                <div className="mt-2 text-3xl font-bold text-slate-950">
                  {compCollectionRate != null ? `${compCollectionRate}%` : "—"}
                </div>
              </div>
            </div>

            <div className="grid gap-6 xl:grid-cols-2">
              {compStatusSlices.length > 0 && (
                <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                  <h3 className="mb-4 text-base font-semibold text-slate-900">Entry status breakdown</h3>
                  <DonutChart slices={compStatusSlices} colours={["#10b981", "#f59e0b", "#64748b"]} />
                </div>
              )}
              {compFeeRows.length > 0 && (
                <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                  <h3 className="mb-4 text-base font-semibold text-slate-900">Entry fees by competition</h3>
                  <HBarChart rows={compFeeRows} colour="#f97316" />
                </div>
              )}
            </div>
            ...
            <div className="rounded-2xl border border-slate-200 bg-white shadow-sm">
              <div className="flex items-center justify-between border-b border-slate-100 px-6 py-4">
                <h3 className="text-base font-semibold text-slate-900">Competition entries with fees ({compEntriesWithFee.length})</h3>
                <ExportButton data={compEntriesWithFee} filename="competition-entry-fees.csv" columns={compExportColumns} />
              </div>
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-slate-100 text-sm">
                  <thead className="bg-slate-50 text-xs font-semibold uppercase tracking-wide text-slate-500">
                    <tr>
                      {["Competition", "Entrant", "Status", "Entry Fee", "Submitted"].map((h) => (
                        <th key={h} className="px-4 py-3 text-left">{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100">
                    {compEntriesWithFee.slice(0, 100).map((e) => (
                      <tr key={e.id} className="hover:bg-slate-50">
                        ...
                        <td className="px-4 py-2 font-semibold text-slate-900">
                          £{parseFloat(e.entryFee).toFixed(2)}
                        </td>
                        <td className="px-4 py-2 text-slate-500">
                    ...
                    {compEntriesWithFee.length > 100 && (
                      <tr>
                        <td colSpan={5} className="px-4 py-3 text-center text-xs text-slate-400">
                          Showing 100 of {compEntriesWithFee.length} — export CSV for full data
                        </td>
                      </tr>
                    )}
                  </tbody>
