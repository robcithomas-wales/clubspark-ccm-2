...
export async function POST(_request: NextRequest, { params }: { params: Promise<{ id: string; fixtureId: string }> }) {
  ...
  try {
    const res = await fetch(`${TEAM_SERVICE}/teams/${id}/fixtures/${fixtureId}/availability/request`, {
      method: "POST",
    ...
    return NextResponse.json({ error: "Failed to request availability" }, { status: 500 })
  }
}
