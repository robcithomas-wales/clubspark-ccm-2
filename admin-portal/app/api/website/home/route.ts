...
export async function GET() {
  try {
    ...
    // Return just the homePageContent portion
    return NextResponse.json({ data: json?.data?.homePageContent ?? {} }, { status: res.status })
  } catch (error) {
...
export async function PATCH(request: Request) {
  try {
    ...
    const res = await fetch(`${VENUE_SERVICE}/organisations/me/home-page`, {
      method: "PATCH",
      headers,
