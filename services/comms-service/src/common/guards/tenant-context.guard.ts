...
interface SupabasePayload {
  sub: string
  app_metadata?: { tenantId?: string; organisationId?: string }
}
...
export class TenantContextGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if (authHeader?.startsWith('Bearer ')) {
      ...
      if (!tenantId) throw new UnauthorizedException('Token is missing tenantId claim')
      request.tenantContext = { tenantId, organisationId: payload.app_metadata?.organisationId }
      return true
    }
