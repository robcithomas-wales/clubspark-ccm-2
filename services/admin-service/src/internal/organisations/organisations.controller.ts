import {
  Controller, Get, Post, Patch, Param, Body, Query, HttpCode, HttpStatus, UseGuards, Request,
} from '@nestjs/common'
...
export class OrganisationsController {
  ...
  async list(
    ...
    const result = await this.service.list(Number(page), Number(limit), { search, status, plan, region })
    return {
      data: result.data,
      pagination: {
        total: result.total,
  ...
  async getDetail(@Request() req: InternalReq, @Param('tenantId') tenantId: string) {
    ...
    void this.audit.log(req.internalContext, tenantId, 'organisation.viewed', 'organisation', tenantId)
    return { data }
  }
  ...
  async create(@Request() req: InternalReq, @Body() dto: CreateOrganisationDto) {
    ...
    void this.audit.log(req.internalContext, dto.tenantId, 'organisation.created', 'organisation', data.id, { name: dto.name })
    return { data }
  }

  @Post('sync')
  @HttpCode(HttpStatus.OK)
  @ApiOperation({ summary: 'Upsert org profile from venue-service (name/slug/email only, preserves plan/status)' })
  ...
  async update(
    ...
    void this.audit.log(req.internalContext, tenantId, 'organisation.updated', 'organisation', tenantId, dto as Record<string, unknown>)
    return { data }
  }
}
