...
export class TenantContextGuard implements CanActivate {
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if (authHeader?.startsWith('Bearer ')) {
      ...
      request.tenantContext = { userId: payload.sub, tenantId, organisationId }
      return true
    }

    // Header fallback for integration tests
    ...
    request.tenantContext = { userId: userId ?? 'test-user', tenantId, organisationId }
    return true
  }
}
