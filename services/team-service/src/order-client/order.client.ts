 ...
 * PRODUCTION: replace with Azure Service Bus message or gRPC.
 */
@Injectable()
export class OrderClient {
  ...
  async createOrder(payload: CreateOrderPayload): Promise<OrderResponse | null> {
    try {
      ...
      if (!res.ok) {
        this.logger.warn(`order-service returned ${res.status} for charge run ${payload.subjectId}`)
        return null
      }

      return res.json() as Promise<OrderResponse>
    } catch (err) {
      // Non-fatal: order creation failure must not block charge run creation during pilot
      this.logger.error(`Failed to create order for charge run ${payload.subjectId}: ${String(err)}`)
      return null
    }
  }
}
