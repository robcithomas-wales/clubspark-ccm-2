...
export class JobLeaseService {
  ...
  async tryAcquire(jobName: string, leaseSeconds: number): Promise<JobLease | null> {
    ...
    const rows = await this.prisma.$queryRaw<{ jobName: string }[]>`
      INSERT INTO membership.scheduled_job_leases AS lease (job_name, owner_id, lease_until)
  ...
  async release(lease: JobLease): Promise<void> {
    await this.prisma.$executeRaw`
      DELETE FROM membership.scheduled_job_leases
