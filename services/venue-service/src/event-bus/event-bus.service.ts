...
export interface VenueProjectionEvent {
  eventId: string
  type:
...
export class EventBusService {
  ...
  constructor(config: ConfigService<AppConfig, true>) {
    this.bookingUrl = `${config.get('bookingService', { infer: true }).url}/booking-projections/internal/venue/events`
  }

  async publish(event: VenueProjectionEvent): Promise<void> {
    ...
    if (!response.ok) {
      throw new Error(`Booking projection consumer returned ${response.status}`)
    }
