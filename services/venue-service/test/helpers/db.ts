...
export async function seedFixtures(): Promise<void> {
  ...
  await prisma.$executeRaw`
    INSERT INTO venue.resources (id, tenant_id, venue_id, name, resource_type, is_active, updated_at)
    VALUES (${TEST_RESOURCE_ID}::uuid, ${TEST_TENANT_ID}::uuid, ${TEST_VENUE_ID}::uuid, 'Test Court 1', 'court', true, NOW())
    ON CONFLICT (id) DO NOTHING
  `
}
...
export async function teardownFixtures(): Promise<void> {
  ...
  await prisma.$executeRaw`
    ...
    WHERE unit_id IN (SELECT id FROM venue.bookable_units WHERE tenant_id = ${TEST_TENANT_ID}::uuid)
       OR conflicting_unit_id IN (SELECT id FROM venue.bookable_units WHERE tenant_id = ${TEST_TENANT_ID}::uuid)
  `
  await prisma.$executeRaw`
