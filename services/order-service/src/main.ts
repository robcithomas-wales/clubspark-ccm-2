...
async function bootstrap(): Promise<void> {
  ...
  if (nodeEnv !== 'production') {
    const swaggerConfig = new DocumentBuilder()
      .setTitle('ClubSpark — Order Service')
      .setDescription('Shared commerce layer: orders and order items across all activity domains')
      .setVersion('1.0')
  ...
  logger.log(`Order service running on http://localhost:${port} [${nodeEnv}]`)
}
