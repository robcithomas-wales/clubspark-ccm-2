...
export async function POST(request: NextRequest, { params }: { params: Promise<{ tenantId: string }> }) {
  ...
  try {
    const res = await fetch(`${ADMIN_SERVICE}/v1/internal/organisations/${tenantId}/impersonate`, {
      method: "POST", headers: await internalHeaders(), body: JSON.stringify(await request.json()),
    })
