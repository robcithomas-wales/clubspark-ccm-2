...
export async function PUT(
  ...
  try {
    const res = await fetch(`${ADMIN_SERVICE}/v1/internal/organisations/${tenantId}/flags/${flag}`, {
      method: "PUT", headers: await internalHeaders(), body: JSON.stringify(await request.json()),
    })
...
export async function DELETE(
  ...
  try {
    const res = await fetch(`${ADMIN_SERVICE}/v1/internal/organisations/${tenantId}/flags/${flag}`, {
      method: "DELETE", headers: await internalHeaders(),
    })
